...
#set wd
setwd("/home/spr19/Desktop/PHC6194SPR2019/WK12")
...
mod$summary.random #the first 32 rows include information on the spatially unstructured residual nu (the primary interest in a disease mapping study), and the last 32 rows include information on the spatially structured residual upsilon only

#calculate zeta=exp(nu)
m <- mod$marginals.random$ID[1:Nareas]  #a marginal object with density values y at locations x
...
#obtain the proportion of variance explained by the spatially structured component upsilon, taking the structured effect upsilon and calculating the empirical variance 
#create a matrix with rows equal to the number of areas and 1000 columns. Then for each area we extract 1000 values from the corresponding marginal distribution of upsilon and finally we calculate the empirical variance. We also extract the expected value of the variance for the unstructured component and build the spatial fractional variance
